...
void logo_to_framebuffer(void)
{
  ...
  for (page = 0; page <= 7; page++)//each page is one 8th of the height, or 8 bits, or 2 hex numbers
  {
    for (column = 0; column < 128 / 8; column++) //there are 16 hex columns
    {
      for (i = 0; i < 8; i++)
      {
        pixels = 0;
...
//============================================================================
void process_brightness_map(uint8_t x[8][128])
{
  uint8_t
    column;
  uint8_t
  ...
  uint8_t theByte;
  uint16_t coordinate;
  for (column = 0; column < 128; column++) //there are 16 double-hex columns
    {
      for (rowpage = 0; rowpage < 8; rowpage++) //hex value for 8 bits in a row
      {
        pixels = 0;
      
          
          // Read the byte
        pixels = x[rowpage][column];
        framebuffer[rowpage][column] = pixels; //8 by 128
      }
    }
